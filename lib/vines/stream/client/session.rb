  ...
  class Stream
    class Client
      ...
      class Session
        ...
        # roster pushes when changes are made to their followers.
        ...
        def available_subscribed_to_resources
          subscribed = @user.subscribed_to_followers.map {|c| c.jid }
        ...
        def available_subscribers
          subscribed = @user.subscribed_from_followers.map {|c| c.jid }
          router.available_resources(subscribed, @user.jid)
        end

        # Returns followers hosted at remote servers to which this user has
        # successfully subscribed.
        def remote_subscribed_to_followers
          @user.subscribed_to_followers.reject do |c|
            @config.local_jid?(c.jid)
          end
        end

        # Returns followers hosted at remote servers that are subscribed
        # to this user's presence updates.
        def remote_subscribers(to=nil)
          jid = (to.nil? || to.empty?) ? nil : JID.new(to).bare
          @user.subscribed_from_followers.reject do |c|
        ...
        def broadcast_unavailable
          ...
          Fiber.new do
            ...
            remote_subscribers.each do |follower|
              node = unavailable
